...
err_t WyzBeeI2c_Init( WyzBeeI2c_Config_t    *p_config )
{
	...
	if(p_config  !=  NULL) 												// check for valid pointer
	{
		...
		if(retval == Ok)
		{
			Mfs_SetTxIntCallBack (I2C_INSTANCE , (mfs_tx_cb_func_ptr_t)WyzBeeI2c_TxIsr   );	// Register I2c TX ISR
			Mfs_SetRxIntCallBack (I2C_INSTANCE , (mfs_tx_cb_func_ptr_t)WyzBeeI2c_RxIsr   );	// Register I2c RX ISR
			Mfs_SetStsIntCallBack(I2C_INSTANCE , (mfs_tx_cb_func_ptr_t)WyzBeeI2c_SatusIsr);	// Register I2c Status ISR
