...
en_result_t Reset_GetCause(stc_reset_result_t* pstcResult)
{
  ...
  stcReadResetCause = ((FM4_CRG_TypeDef *)((0x40000000UL) + 0x10000UL))->RST_STR_f;

  
  if (1 == stcReadResetCause.PONR)
  {
    pstcResult->bPowerOn = ((boolean_t) 1);
  }
  else
  {
    pstcResult->bPowerOn = ((boolean_t) 0);
  }
  
  
  if (1 == stcReadResetCause.INITX)
  {
    pstcResult->bInitx = ((boolean_t) 1);
  }
  else
  {
    pstcResult->bInitx = ((boolean_t) 0);
  }
  
  
  if (1 == stcReadResetCause.LVDH)
  {
    pstcResult->bLowVoltageDetection = ((boolean_t) 1);
  }
  else
  {
    pstcResult->bLowVoltageDetection = ((boolean_t) 0);
  }
  
  
  if (1 == stcReadResetCause.SWDT)
  {
    pstcResult->bSoftwareWatchdog = ((boolean_t) 1);
  }
  else
  {
    pstcResult->bSoftwareWatchdog = ((boolean_t) 0);
  }
  
  
  if (1 == stcReadResetCause.HWDT)
  {
    pstcResult->bHardwareWatchdog = ((boolean_t) 1);
  }
  else
  {
    pstcResult->bHardwareWatchdog = ((boolean_t) 0);
  }
  
  
  if (1 == stcReadResetCause.CSVR)
  {
    pstcResult->bClockSupervisor = ((boolean_t) 1);
  }
  else
  {
    pstcResult->bClockSupervisor = ((boolean_t) 0);
  }
  
  
  if (1 == stcReadResetCause.FCSR)
  {
    pstcResult->bAnomalousFrequency = ((boolean_t) 1);
  }
  else
  {
    pstcResult->bAnomalousFrequency = ((boolean_t) 0);
  }
  
  
  if (1 == stcReadResetCause.SRST)
  {
    pstcResult->bSoftware = ((boolean_t) 1);
  }
  else
  {
    pstcResult->bSoftware = ((boolean_t) 0);
  }
