...
INT16 rsi_bt_spp_app_transfer(RSI_BT_CMD_SPP_TRANSFER *uSPPTransfer)
{
  INT16          retval;
	UINT08         rsi_frameCmdSPPTransfer1[4] = {0};

#ifdef RSI_DEBUG_PRINT
  RSI_DPRINT(RSI_PL3,"\r\n\nSPP Transfer Start ");
#endif
	
	rsi_frameCmdSPPTransfer1[0] = (INT08) uSPPTransfer->SppTransferFrameSnd.DataLength + 2;//strlen (uSPPTransfer->SppTransferFrameSnd.Data) + 2;
	rsi_frameCmdSPPTransfer1[1] = 0;
	rsi_frameCmdSPPTransfer1[2] = 0x21;
	rsi_frameCmdSPPTransfer1[3] = 0;

  retval = rsi_bt_execute_cmd((UINT08 *)rsi_frameCmdSPPTransfer1, (UINT08 *)uSPPTransfer, uSPPTransfer->SppTransferFrameSnd.DataLength + 2);
  return retval;
}
