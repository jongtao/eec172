...
/*=============================================================================*/
/**
 * @fn            uint32 rsi_bytes4_to_uint32(uint8 *dBuf)
...
/*=============================================================================*/
/**
 * @fn            uint16 rsi_bytes2_to_uint16(uint8 *dBuf)
 ...
 */
void register_socket_protocol(rsi_api  *ptrStrApi)
{

  uint16 socket_descriptor = 0;
#ifdef RSI_LITTLE_ENDIAN
  socket_descriptor = *(uint16 *)(rsi_app_cb.uCmdRspFrame->uCmdRspPayLoad.socketFrameRcv.socketDescriptor);
#else
  ...
  if(*(uint16 *)rsi_app_cb.uCmdRspFrame->uCmdRspPayLoad.socketFrameRcv.ip_version == IP_VERSION_4)
  {
    ...
    memcpy((uint8 *)rsi_app_cb.socketsStrArray.socketsArray[socket_descriptor].dest_ip.ipv4_address, ptrStrApi->uSocketFrame.socketFrameSnd.destIpaddr.ipv4_address, 4);
  ...
  else
  {
    ...
    memcpy((uint8 *)rsi_app_cb.socketsStrArray.socketsArray[socket_descriptor].dest_ip.ipv6_address, ptrStrApi->uSocketFrame.socketFrameSnd.destIpaddr.ipv6_address, 16);
    /* Tag the socket with its protocol in the sockets array structure */
    if (rsi_bytes2R_to_uint16(rsi_app_cb.uCmdRspFrame->uCmdRspPayLoad.socketFrameRcv.socketType) == RSI_SOCKET_TCP_CLIENT) {
  ...
  //! destination port
  memcpy((uint8 *)rsi_app_cb.socketsStrArray.socketsArray[socket_descriptor].dest_port, ptrStrApi->uSocketFrame.socketFrameSnd.destSocket, 2);
}
...
void unregister_socket_protocol(void)
{
  ...
  uint16 socket_descriptor = 0;
#ifdef RSI_LITTLE_ENDIAN
  socket_descriptor = *(uint16 *)(rsi_app_cb.uCmdRspFrame->uCmdRspPayLoad.socketCloseFrameRcv.socketDsc);
#else
  socketDescriptor  = rsi_bytes2R_to_uint16(rsi_app_cb.uCmdRspFrame->uCmdRspPayLoad.socketFrameRcv.socketDsc);
#endif
...
/*=========================================================================*/
/**
 * @fn          void parse_ipv6_address(uint32 *v6,uint8 *buf)
