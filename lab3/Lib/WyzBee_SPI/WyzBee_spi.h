...
typedef struct WyzBeeSpi_Config								/*-------------SPI CONFIG STRUCTURE--------*/
{
	...
	int8_t        				BitDirection;   	// MSB first or MSB first see description of #WyzBeeSpi_BitOrde_t
	int8_t        				ClockLvl;       	// Clock Level (LOW/HIGH)
	int8_t       		 	 	UseInterrupts;  	// Enable Disable Interrupts
...
*********************************************************************************************************
*/
uint16_t   WyzBeeSpi_Init		 	( WyzBeeSpi_Config_t      *p_config );

uint16_t   WyzBeeSpi_SyncSend     	( uint8_t 				  *p_buf,
							      uint16_t 	 			   buf_len  );

uint16_t   WyzBeeSpi_SyncSendRecv 	( uint8_t 				  *p_tx_buf,
							      uint16_t   			       tx_len,
							      uint8_t 				  *p_rx_buf,
							      uint16_t   			   rx_len  	);

uint16_t   WyzBeeSpi_SyncRecv	 	( uint8_t 				  *buf,
							      uint16_t    			   buf_len   );

uint16_t   WyzBeeSpi_Write		 	( uint8_t 				   tx_char   );

uint16_t   WyzBeeSpi_DeInit		( void                            	);

void    WyzBeeSpi_CsEnable	 	( void                            	);

void    WyzBeeSpi_CsDisable    	( void                            	);

uint16_t   WyzBeeSpi_TxIntrEnable	( void                            	);

uint16_t   WyzBeeSpi_TxIntrDisable	( void                            	);

uint16_t   WyzBeeSpi_RxIntrEnable 	( void                            	);

uint16_t   WyzBeeSpi_RxIntrDisable ( void                            	);

uint8_t WyzBeeSpi_Read		 	( void                            	);

uint16_t   WyzBeeSpi_RxEnable	 	( void                            	);

uint16_t   WyzBeeSpi_TxEnable	 	( void                            	);

uint16_t   WyzBeeSpi_RxDisable	 	( void                            	);

uint16_t   WyzBeeSpi_TxDisable	 	( void                            	);
