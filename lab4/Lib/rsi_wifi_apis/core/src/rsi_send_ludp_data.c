 ...
 * @return      errCode
 * @return      -2 = Command execution failure
    ...
    uint32 payloadLen,uint8 protocol, uint8 *destIp, uint16 destPort,uint32 *bytes_sent)
{
  ...
  *(uint16 *)rsi_app_cb.uSendFrame.sendFrameLudpSnd.sendDataOffsetSize = (uint16)headerLen;
#else
  rsi_uint16_to_2bytes(rsi_app_cb.uSendFrame.sendFrameLudpSnd.sendDataOffsetSize, headerLen);
#endif

#ifdef RSI_LITTLE_ENDIAN
  *(uint16 *)rsi_app_cb.uSendFrame.sendFrameLudpSnd.ip_version = RSI_IP_VERSION;
  *(uint16 *)rsi_app_cb.uSendFrame.sendFrameLudpSnd.socketDescriptor = socketDescriptor;
  *(uint16 *)rsi_app_cb.uSendFrame.sendFrameLudpSnd.destPort = destPort;
#else
  ...
  if(*(uint16 *)rsi_app_cb.uSendFrame.sendFrameLudpSnd.ip_version == IP_VERSION_4)
  {
...
#ifdef RSI_LITTLE_ENDIAN
    *(uint16 *)rsi_app_cb.uSendFrame.sendFrameLudpSnd.sendBufLen = (uint16)send_payload_len;
#else
