...
err_t WyzBeeI2c_Init( WyzBeeI2c_Config_t    *p_config )
{
	...
	if(p_config  !=  0) 												
	{
		...
		(void)Mfs_Csio_DeInit(&MFS7); 						 	
																		 
		retval = Mfs_I2c_Init(&MFS7, (stc_mfs_i2c_config_t *)&config_internal);

		if(retval == Ok)
		{
			Mfs_SetTxIntCallBack (&MFS7 , WyzBeeI2c_TxIsr   );	
			Mfs_SetRxIntCallBack (&MFS7 , WyzBeeI2c_RxIsr   );	
			Mfs_SetStsIntCallBack(&MFS7 , WyzBeeI2c_SatusIsr);	
			Mfs_InitTxIrq(&MFS7);								
			Mfs_InitRxIrq(&MFS7);								
						...
						uint16_t   buf_len)
{
	...
	if( (p_buf != 0) && (buf_len != 0u) )  
	{
		...
		Mfs_WriteData(&MFS7, (uint16_t)(((uint8_t)slave_addr << 1) | I2cWrite));

		unIBCR.u8IBCR           =  Mfs_GetIBCR(&MFS7);			
		...
		Mfs_SetIBCR(&MFS7, unIBCR.u8IBCR);						

		unSMR.u8SMR             =  Mfs_GetSMR(&MFS7);			
		unSMR.stcSMR.TIE        =  1u;								

		Mfs_SetSMR(&MFS7, unSMR.u8SMR);							
					 ...
					 uint16_t  *p_buf_len)
{
	...
	Mfs_WriteData(&MFS7, (uint16_t)(((uint8_t)slave_addr << 1) | I2cRead));
...
static void WyzBeeI2c_TxIsr(void)
{
	...
	unIBSR.u8IBSR = Mfs_GetIBSR(&MFS7);
	unIBCR.u8IBCR = Mfs_GetIBCR(&MFS7);

	if (1u == unIBSR.stcIBSR.SPC) 									
	{
		unIBSR.stcIBSR.SPC     = 0u; 								
		Mfs_SetIBSR(&MFS7 , unIBSR.u8IBSR);
		...
		unISMK.u8ISMK          = Mfs_GetISMK(&MFS7); 			
		unISMK.stcISMK.EN      = 0u;
		Mfs_SetISMK(&MFS7 ,unISMK.u8ISMK);
	...
	{
		unSSR.u8SSR            = Mfs_GetSSR(&MFS7);
		if (1u == unSSR.stcSSR.TDRE)
		{																
			Mfs_WriteData(&MFS7, (uint16_t)GlobalBuf[TxBufIndex]);
			TxBufIndex++;
...
static void WyzBeeI2c_RxIsr(void)
{
	...
	else if ((1u == unSSR.stcSSR.RDRF) && (0u == unIBSR.stcIBSR.FBT))
	{
		while (RxBufIndex < GlobalBufLen) 								
		{
			...
			{
				GlobalBuf[RxBufIndex] = (uint8_t)Mfs_ReadData(&MFS7);
				RxBufIndex++;
			}
			else
			{
