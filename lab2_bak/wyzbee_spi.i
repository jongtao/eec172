...
err_t WyzBeeSpi_Init(WyzBeeSpi_Config_t   *p_config)
{
	...
	res=Mfs_Csio_Init(&MFS4, (stc_mfs_csio_config_t *)&config_internal);
	if(Ok != res)													
	{
	...
	Mfs_Csio_SetSckOutEnable(&MFS4, 1u);					
	Mfs_ErrorClear(&MFS4);									
	WyzBeeSpi_TxEnable();											
	WyzBeeSpi_RxEnable();											
	...
	{
		Mfs_SetTxIntCallBack(&MFS4, p_config->TxIsr);		
		Mfs_SetRxIntCallBack(&MFS4, p_config->RxIsr);		
...
err_t WyzBeeSpi_SyncSend(uint8_t *p_buf , uint16_t buf_len)
{
	...
	if((p_buf!=0) && (buf_len!=0))								
	{
		...
		while(buf_len!=0)
		{
			do
			{														
				tx_status = Mfs_GetStatus(&MFS4, MFS_CSIO_SSR_TDRE);
	...
	if((p_tx_buf!=0) && (p_rx_buf !=0) && (tx_len!=0) && (rx_len!=0))
	{
		...
		while(MaxTxRxCnt!=0)
		{
			...
			if(rx_buf_indx <= rx_len)
			{
				do
				{													
					rx_status = Mfs_GetStatus(&MFS4, MFS_CSIO_SSR_RDRF);
...
err_t WyzBeeSpi_TxIntrEnable(void)
{
	...
	res = Mfs_SetTxIntEnable(&MFS4, 1u);					
	if(res==Ok)
	{
		return 0u;
	}
...
err_t WyzBeeSpi_TxIntrDisable(void)
{
	...
	res = Mfs_SetTxIntEnable(&MFS4, 0u);					
	if(res == Ok)
	{
		return 0u;
	}
...
err_t WyzBeeSpi_RxIntrEnable(void)
{
	...
	res = Mfs_SetRxIntEnable(&MFS4, 1u);					
	if(res == Ok)
	{
		return 0u;
	}
...
err_t WyzBeeSpi_RxIntrDisable(void)
{
	...
	res = Mfs_SetRxIntEnable(&MFS4, 0u);					
	if(res == Ok)
	{
		return 0u;
	}
...
err_t WyzBeeSpi_Write(uint8_t tx_char)
{
	...
	res = Mfs_WriteData(&MFS4, send_char);						
	if(res == Ok)
	{
		return 0u;
	}
